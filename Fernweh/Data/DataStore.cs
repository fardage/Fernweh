...
namespace Fernweh.Data
{
    public static class DataStore
    {
        ...
        {
            using (var tripContext = new TravelContext())
            {
        ...
        private static void UpdateRemoteByTripId(string tripId)
        {
            using (var travelContext = new TravelContext())
            {
                ...
                if (trip.IsShared)
                {
                    ...
                    _ = _serialQueue.Enqueue(async () =>
                    {
                        var remoteTrip = await restService.SaveTripAsync(trip);
                        await UpdateTripAsync(remoteTrip);
                    });
                }
            }
        }
        ...
        private static async Task UpdateLocalTripsAsync()
        {
            using (var travelContext = new TravelContext())
            {
                var trips = travelContext.Trips.ToList();
                foreach (var trip in trips.Where(t => t.IsShared))
                {
                    var restService = new RestService.RestService();
                    var response = await restService.GetTripAsync(trip.Id);
                    response.IsShared = true;
                    travelContext.Update(trip);
                    await travelContext.SaveChangesAsync();
                }
            }
